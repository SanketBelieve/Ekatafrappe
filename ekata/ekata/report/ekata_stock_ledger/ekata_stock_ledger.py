# File: your_app/your_app/report/ekata_stock_ledger/ekata_stock_ledger.py

# Copyright (c) 2015, Frappe Technologies Pvt.
...
def execute(filters=None):
    ...
    # Pull columns and items (now with custom filter)
    ...
    actual_qty = opening_row.get("qty_after_transaction", 0) if opening_row else 0
    stock_value = opening_row.get("stock_value", 0) if opening_row else 0
    ...
    for sle in sl_entries:
        detail = item_details.get(sle.item_code, {})
        sle.update(detail)
        ...
        if include_uom:
            conversion_factors.append(detail.get("conversion_factor", 1))
...
def get_columns(filters):
    columns = [
        ...
        # New column for custom field
        {"label": _("Local Item Name"), "fieldname": "custom_item_local_name", "fieldtype": "Data", "width": 120},
    ]
    ...
    columns.extend([
        ...
        {"label": _("Avg Rate (Balance Stock)"), "fieldname": "valuation_rate", "fieldtype": filters.valuation_field_type, "width": 180},
        {"label": _("Valuation Rate"), "fieldname": "in_out_rate", "fieldtype": filters.valuation_field_type, "width": 140},
...
def get_items(filters):
    ...
    if filters.get("item_code"):
        conditions.append(item.name == filters["item_code"])
    else:
        if filters.get("brand"):
            conditions.append(item.brand == filters["brand"])
        if filters.get("item_group"):
            cond = get_item_group_condition(filters["item_group"], item)
            if cond:
                conditions.append(cond)

    # New filter condition
    if filters.get("custom_item_local_name"):
        conditions.append(item.custom_item_local_name == filters["custom_item_local_name"])
...
def get_item_details(items, sl_entries, include_uom):
    item_details = {}
    if not items:
        items = list({d.item_code for d in sl_entries})
    if not items:
        return item_details
