    ...
    for company, item, warehouse in sorted(iwb_map):
        if item_map.get(item):
            ...
            if item + warehouse in item_reorder_detail_map:
                item_reorder_level = item_reorder_detail_map[item + warehouse][
                    "warehouse_reorder_level"
                ]
                item_reorder_qty = item_reorder_detail_map[item + warehouse][
...
def get_item_warehouse_map(filters, sle):
    ...
    for d in sle:
        ...
        if d.posting_date < from_date or d.voucher_no in opening_vouchers.get(
            d.voucher_type, []
        ):
