...
class CustomNamingSeries(Document):
    ...
    def check_duplicate(self):
        ...
        sr = [
            [frappe.get_meta(p).get_field("naming_series").options, p] for p in parent
        ]
            ...
            for i in sr:
                if i[0]:
                    ...
                    if series.split(".")[0] in existing_series:
                        frappe.throw(
                            _("Series {0} already used in {1}").format(series, i[1])
                        )
    ...
    def get_options(self, arg=None):
        if frappe.get_meta(arg or self.select_doc_for_series).get_field(
            "naming_series"
        ):
            return (
                frappe.get_meta(arg or self.select_doc_for_series)
    ...
    def get_current(self, arg=None):
        ...
        if self.prefix:
            ...
            self.current_value = frappe.db.get_value(
